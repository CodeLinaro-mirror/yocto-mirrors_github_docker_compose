...
//	    [→ kill (signal=SIGKILL) when the grace period expires]
//	    → stop + die (exitCode)
...
// show up in the stream but don't affect container tracking. Note that exec_*
// and health_status actions embed variable data in the action name itself
// (e.g. "health_status: healthy"), which would break exact matching. On
// daemon shutdown containers are stopped (kill/die/stop) but the event stream
// usually breaks before delivering those events, so a daemon restart is
// observed as Events returning an error, not as per-container sequences. A
// container that dies while the daemon itself is down never gets a die event
// at all: on daemon restore it is just recorded as exited, and containers
// the restore restarts per policy only emit start.
